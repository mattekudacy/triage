...
- **Five measured charts in `README.md`, generated by `scripts/gen_readme_charts.py`** — the
  ...
  The generator is stdlib-only (hand-written SVG, no matplotlib, no new dependency) and does
  not transcribe the README's tables: it re-scores corpora A-E through the same
  `RulesClassifier` path `scripts/classifier_accuracy.py` uses and derives the routing demo
  from `bench_synthetic.py`'s own `_task_body`, so a `rules.py` change that moves a held-out
  number moves the charts on the next run and the two cannot silently disagree. The two
  figures that genuinely cannot be recomputed — corpus C's pre-tuning v1.0 score and the
  `LLMClassifier`/`HybridClassifier` runs, which need a live model — are marked `HISTORICAL`
  constants with the document that recorded them. Each SVG carries its own
  `prefers-color-scheme` block, so one asset reads correctly in both GitHub themes. Also adds
  the corpus E row (69% recall, 100% precision) to the README's block table, which had stopped
  at corpus D, and corrects "Nine-block measurement" to ten.
