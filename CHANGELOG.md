...
- **README cut from ~860 lines to ~510** — it had grown into a second copy of the docs site
  (full checkpoint/hooks/observability/budget walkthroughs already covered in
  `docs/concepts/*.md` and `docs/api/*.md`) plus a near-duplicate of `known-limitations.md`'s
  entire accuracy narrative. The Results section now leads with one chart
  (`heldout-recall-by-group.png`) and the headline numbers, with the other four
  charts and the full corpus-by-corpus breakdown moved into `docs/known-limitations.md`
  next to the prose that already unpacks them — same content, no longer told twice.
  Sections with a dedicated docs page (failure policy, strategies, checkpoints, attempt
  history, custom classifiers) are now a short summary + link instead of a full walkthrough;
  sections with no other home (human-in-the-loop, lifecycle hooks, escalation handling) are
  unchanged. Adds a "Learn more" table linking the full site. `docs/concepts/classifiers.md`
  also gained a back-link to Known Limitations' held-out numbers next to its own accuracy
  section, closing a gap where that page cited only the rosy training-data figure.

### Removed

- **`examples/benchmark.py`** — the v0.14-era synthetic accuracy suite (26 self-authored
  cases, 100% by construction) that `docs/concepts/classifiers.md` cited as "Accuracy on the
  synthetic suite" with no link to the real held-out numbers corpora A-E now provide. Kept
  alongside the corpus-based measurement, it read as a second, rosier accuracy claim rather
  than a stale one. `docs/concepts/classifiers.md` and `docs/known-limitations.md` now point
  to `scripts/classifier_accuracy.py` and the held-out corpora instead.
- **`ROADMAP.md`** — its "Done" table stopped at v0.24 and its "Next" section still listed
  multi-agent failure detection as unstarted work, when `docs/concepts/multi-agent-failures.md`
  already tracks that in detail (phases 1-2 shipped, phase 3 scoping underway) alongside every
  other post-v0.24 change already in `CHANGELOG.md`. Two trackers for the same roadmap had
  drifted out of sync; keeping one is safer than updating two going forward.
- **`docs/plans/2026-05-18-step-risk-scoring.md`** — an implementation plan for work that has
  already shipped, per `mkdocs.yml`'s own comment on why it was excluded from the published
  site. Excluding it from the site while still shipping it as a shipped-work artifact in the
  repo's source tree was two states for the same fact; deleted the artifact along with the
  now-empty `exclude_docs` entry in `mkdocs.yml`.

### Added

- **5 missing example doc pages**: `docs/examples/langgraph-agent.md`, `multi-agent.md`,
  `policy-chain.md`, `policy-sequence.md`, `otel-tracing.md`. The corresponding
  `examples/*.py` files existed and worked but weren't linked from any doc page, the mkdocs
  nav, or the README — undiscoverable unless you already knew to look in the `examples/`
  directory. Wired into `mkdocs.yml`'s Examples nav alongside the existing pages.
