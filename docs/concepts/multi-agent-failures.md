...
**Status: phases 1 and 2 done (see "Recommended phasing" below — phase 2's outcome is a negative result, not shipped code); phase 3 scoping has started (one low-risk prerequisite shipped, a pilot corpus + experimental scoring script sourced for 3 of 12 modes — see "Phase 3 scoping" below), but the actual LLM measurement is blocked on an API key this environment doesn't have and hasn't been run yet.** This document exists to answer one question concretely before writing code: what would it actually take for `triage` to detect multi-agent failure modes, and how much of that can be done honestly with the same rigor the rest of this project holds itself to — measured, not assumed, and never a new `FailureType` without real disambiguation logic behind it.
...
### ✅ Pilot corpus sourced (2.5, 2.4, 1.4) — measurement itself still blocked on an API key

Following the recommendation above, `scripts/gen_mast_pilot_corpus.py` sourced real, cited examples for exactly those three modes into `tests/data/mast_pilot_corpus.json`, and `scripts/mast_mode_pilot_accuracy.py` scores an experimental prompt against it — entirely outside `LLMClassifier`'s stable contract, per this section's own proposed ordering. Neither script has been run against a real model yet: like `scripts/hybrid_ambiguity_accuracy.py`, this requires an LLM API key this environment doesn't have, so **no accuracy number is claimed here** — only what the sourcing itself found.

**2.5 Ignored Other Agent's Input** — two clean, independently-sourced entries: a worker agent processing only the raw user message while disregarding its supervisor's explicit reinterpretation ([FlowiseAI/Flowise#3512](https://github.com/FlowiseAI/Flowise/issues/3512)), and AutoGen's `SelectorGroupChatManager` selecting a different agent than the one actually named in a model's final suggestion because its mention-counting doesn't distinguish `<thinking>` reasoning from the real answer ([microsoft/autogen#6891](https://github.com/microsoft/autogen/issues/6891)) — kept despite being a selection-*mechanism* bug rather than deliberate disregard, specifically because the observable trajectory is indistinguishable from 2.5 either way.

**1.4 Loss of Conversation History** — two clean, independently-sourced entries: a single-agent case where an intervening unrelated query silently evicts an earlier-stated fact from memory ([langchain-ai/langgraph#2395](https://github.com/langchain-ai/langgraph/issues/2395)), and a multi-agent case where a handoff hands the receiving agent a context window already too full to complete its task ([github/copilot-cli#1180](https://github.com/github/copilot-cli/issues/1180)) — gradual crowding-out, a different mechanism than 2.1's abrupt-discontinuity citations already in this doc (`microsoft/autogen#1942`, `langchain-ai/langgraph#6064`).

**2.4 Information Withholding — one clean entry, not two, and that gap is itself the finding.** The clean source: a multi-agent handoff where file resources attached to the source agent never transfer to the destination agent, which then can't complete the same file search ([danny-avila/LibreChat#10569](https://github.com/danny-avila/LibreChat/issues/10569)). The second real candidate found while searching — [openai/openai-agents-python#348](https://github.com/openai/openai-agents-python/issues/348), three agents meant to build on each other's code edits, with the final output missing what agents 2 and 3 should have contributed — turned out **genuinely ambiguous** on inspection: the reporter's own account doesn't establish whether agent 2's contribution was withheld from agent 3 (2.4) or agent 3 ignored what agent 2 produced (2.5), and neither does the trace. Rather than force a label to hit the target count, the corpus records this entry with no single `mast_mode`, flagged `ambiguous_with` instead, and `scripts/mast_mode_pilot_accuracy.py` reports it separately rather than scoring it. This is exactly the kind of result the "Sourcing difficulty, honestly" section above predicted: 2.4 and 2.5 aren't just hard to source independently, they can be hard to *tell apart from a real trace even with full information*, which bears directly on whether either one could ever become a reliable stable `FailureType` — a classifier can't be expected to resolve an ambiguity a human reading the same trace can't resolve either.

`tests/test_mast_pilot_corpus.py` guards the corpus's own shape (every entry cites a real source, every scorable entry uses one of the three piloted modes, the 2/2/1 source count is pinned so it can't silently drift) with zero API calls. Running the actual measurement — whether an LLM can tell these three modes apart given the prompt in `mast_mode_pilot_accuracy.py` — needs a real API key and hasn't happened yet; it's next once one is available, same blocked status as `hybrid_ambiguity_accuracy.py`.
...
3. 🔄 **Scoping started, pilot corpus sourced, measurement not yet run.** See "Phase 3 scoping" above. Shipped so far: `Step.agent_id` reaching `LLMClassifier`'s prompt (low-risk prerequisite), plus a pilot corpus (`tests/data/mast_pilot_corpus.json`, 6 entries) and an experimental scoring script (`scripts/mast_mode_pilot_accuracy.py`) for 3 of the 12 semantic-only modes (2.5, 2.4, 1.4) — entirely outside `LLMClassifier`'s stable contract, per this section's own proposed ordering. Sourcing surfaced a real finding along the way: 2.4 and 2.5 can be genuinely ambiguous even from a full, real trace, not just hard to source independently (see "Pilot corpus sourced" above). The actual measurement — can an LLM tell these apart — needs an API key this environment doesn't have and hasn't been run; same blocked status as `hybrid_ambiguity_accuracy.py`. No `FailureType` or `_SYSTEM_PROMPT` change has been made.
